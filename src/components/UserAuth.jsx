...
export default function UserAuth({ onClose, onLogin, initialMode = "choose" }) {
  ...
  const handleSignUp = async () => {
    ...
    if (data.user) {
      ...
      await supabase.from("reviews").update({ user_id: data.user.id, submitter_email: email }).eq("submitter_email", email);
    }
    setLoading(false);
    if (data.session) {
      onLogin(data.user);
  ...
  const handleForgotPassword = async () => {
    ...
    setLoading(false);
    setSuccess("Password reset email sent. Click the link in that email to set a new password.");
  };
  ...
  const titles = { choose: "MY ACCOUNT", login: "LOG IN", signup: "SIGN UP", forgot: "RESET PASSWORD", changePassword: "NEW PASSWORD" };
  const canGoBack = mode !== "choose" && mode !== "changePassword";
      ...
      <div style={{
        ...
        {mode === "signup" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              ...
              <input style={inp} type="text" placeholder="First name + initial e.g. Priya K." value={displayName} onChange={e => setDisplayName(e.target.value)} autoComplete="off" autoFocus />
            </div>
            <div><label style={lbl}>Email</label><input style={inp} type="email" placeholder="your@email.com" value={email} onChange={e => setEmail(e.target.value)} /></div>
        ...
        {mode === "forgot" && !success && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div><label style={lbl}>Email</label><input style={inp} type="email" placeholder="your@email.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleForgotPassword()} /></div>
        ...
        {mode === "changePassword" && !success && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div><label style={lbl}>New password</label><input style={inp} type="password" placeholder="At least 6 characters" value={newPassword} onChange={e => setNewPassword(e.target.value)} /></div>
            <div><label style={lbl}>Confirm password</label><input style={inp} type="password" placeholder="Repeat your new password" value={confirmPw} onChange={e => setConfirmPw(e.target.value)} onKeyDown={e => e.key === "Enter" && handleChangePassword()} /></div>
