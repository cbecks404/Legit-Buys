...
export default function ProfilePage({ user, onClose, onLogout, theme: T }) {
  ...
  const inp = {
    width: "100%", background: T.surface2 ?? "#161616",
    border: `1px solid ${T.border2 ?? "#333"}`, borderRadius: 10,
    padding: "12px 14px", color: T.text ?? "#f0ede8", fontSize: 14,
  ...
  const ReviewCard = ({ r, target }) => {
    ...
    return (
      <div style={{ background: T.surface ?? "#111", border: `1px solid ${accent}44`, borderRadius: 14, padding: "16px 16px 14px", marginBottom: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
          <div>
            ...
            <div style={{ fontFamily: "'LBCardHeader',serif", fontSize: 16, color: T.text ?? "#f0ede8" }}>{r.product}</div>
          </div>
        ...
        {isEditing ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => setEditingId(null)} style={{ flex: 1, background: "transparent", color: T.textMid ?? "#aaa", border: `1px solid ${T.border2 ?? "#333"}`, borderRadius: 99, padding: "11px 0", fontFamily: "'LBBody',sans-serif", fontSize: 13, cursor: "pointer" }}>Cancel</button>
            </div>
          </div>
        ) : (
          <>
            <p style={{ margin: "0 0 10px", fontSize: 13, color: T.textMid ?? "#ccc", fontFamily: "'LBReview',serif", lineHeight: 1.6 }}>{r.review}</p>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                {r.price_range && <span style={{ fontSize: 11, color: T.textDim ?? "#555", fontFamily: "'DM Mono',monospace", background: T.surface2 ?? "#161616", border: `1px solid ${T.border ?? "#1c1c1c"}`, padding: "1px 7px", borderRadius: 99 }}>{priceSymbol(r.price_range)}</span>}
              ...
              <button onClick={() => startEdit(r, target)} style={{ background: "none", border: `1px solid ${T.border2 ?? "#333"}`, borderRadius: 99, padding: "5px 14px", color: T.textMid ?? "#aaa", fontFamily: "'LBBody',sans-serif", fontSize: 11, cursor: "pointer" }}>
                Edit
              </button>
            </div>
          </>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: T.bg ?? "#080808", display: "flex", flexDirection: "column", overflowY: "auto" }}>
      {/* Header */}
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: T.bg ?? "#080808", padding: "env(safe-area-inset-top) 18px 0" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 0 16px", borderBottom: `1px solid ${T.border ?? "#1a1a1a"}` }}>
          <div style={{ fontFamily: "'LBTitle',sans-serif", fontSize: 22, color: T.text ?? "#f0ede8", letterSpacing: ".04em" }}>MY REVIEWS</div>
      ...
      <div style={{ flex: 1, padding: "24px 18px 60px", maxWidth: 520, width: "100%", margin: "0 auto" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: T.textDim ?? "#555", fontSize: 13, fontFamily: "'DM Mono',monospace" }}>Loading…</div>
        ) : (
          <>
            {/* Profile section */}
            <div style={{ background: T.surface ?? "#111", border: `1px solid ${T.border ?? "#1c1c1c"}`, borderRadius: 14, padding: "18px 16px", marginBottom: 28 }}>
              <div style={{ fontSize: 9, fontFamily: "'DM Mono',monospace", color: T.textDim ?? "#555", letterSpacing: ".18em", textTransform: "uppercase", marginBottom: 12 }}>Your profile</div>
              <div style={{ fontSize: 12, color: T.textDim ?? "#555", fontFamily: "'LBBody',sans-serif", marginBottom: 12 }}>{user.email}</div>

              {profile?.display_name && !editingName ? (
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontSize: 9, fontFamily: "'DM Mono',monospace", color: T.textDim ?? "#555", letterSpacing: ".14em", textTransform: "uppercase", marginBottom: 4 }}>Display name</div>
                    <div style={{ fontSize: 15, color: T.text ?? "#f0ede8", fontFamily: "'LBCardHeader',serif" }}>{profile.display_name}</div>
                  </div>
                  <button onClick={() => setEditingName(true)} style={{ background: "none", border: `1px solid ${T.border2 ?? "#333"}`, borderRadius: 99, padding: "5px 14px", color: T.textMid ?? "#aaa", fontFamily: "'LBBody',sans-serif", fontSize: 11, cursor: "pointer" }}>
                    Edit
                  </button>
                </div>
              ) : (
                <div>
                  ...
                  <input style={{ width: "100%", background: T.surface2 ?? "#161616", border: `1px solid ${T.border2 ?? "#333"}`, borderRadius: 10, padding: "12px 14px", color: T.text ?? "#f0ede8", fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "system-ui,sans-serif", marginBottom: 10 }}
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    {editingName && (
                      <button onClick={() => setEditingName(false)} style={{ background: "transparent", color: T.textMid ?? "#aaa", border: `1px solid ${T.border2 ?? "#333"}`, borderRadius: 99, padding: "9px 16px", fontFamily: "'LBBody',sans-serif", fontSize: 12, cursor: "pointer" }}>
                        Cancel
                      </button>
                    )}
                  </div>
            ...
            {myPending.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                <div style={{ fontSize: 9, fontFamily: "'DM Mono',monospace", color: T.textDim ?? "#555", letterSpacing: ".18em", textTransform: "uppercase", marginBottom: 14 }}>
            ...
            <div>
              ...
              {myReviews.length === 0 && myPending.length === 0 && (
                <div style={{ textAlign: "center", padding: "40px 0", color: T.textDim ?? "#555", fontSize: 13, fontFamily: "'LBBody',sans-serif", lineHeight: 1.7 }}>
